...
namespace SpaceTrader.UI.Screens
{
    public class BuyCargoUI : MonoBehaviour, IScreenUI
    {
        ...
        void AdjustQty(int idx, int delta)
        {
            ...
            // Immediately commit the difference
            int diff = newQty - cur;
            if (diff > 0)       CargoSystem.BuyCargo(idx, diff);
            else if (diff < 0)  CargoSystem.SellCargo(idx, -diff, SellCargo);
        ...
        void OnClear()
        {
            // Sell back everything bought this session
            for (int i = 0; i < MaxTradeItem; i++)
            {
                if (_rows[i].PendingQty[0] > 0)
                {
                    CargoSystem.SellCargo(i, _rows[i].PendingQty[0], SellCargo);
